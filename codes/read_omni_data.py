...
save_dir = '/mnt/cephadrius/udel_research/msc/omni/data/processed/v03/'
...
#	d['sc_r'] = dat['heliocentricDistance']
    ...
    df.replace(-1.e+31, np.nan, inplace=True)
    df['vp_r'] = df.vp_m * np.cos(np.deg2rad(df.elevAngle)) * np.cos(
                                                                  np.deg2rad(df.azimuthAngle))
    df['vp_t'] = df.vp_m * np.cos(np.deg2rad(df.elevAngle)) * np.sin(
                                                                  np.deg2rad(df.azimuthAngle))
    ...
    df['zpr'] = 1.e3*db.vp_r + 1e-9*db.br/np.sqrt(mu_0*1.e6*df.np * m_p)
    df['zpt'] = 1.e3*db.vp_t + 1e-9*db.bt/np.sqrt(mu_0*1.e6*df.np * m_p)
    df['zpn'] = 1.e3*db.vp_n + 1e-9*db.bn/np.sqrt(mu_0*1.e6*df.np * m_p)
    df['zpm'] = df.zpr**2 + df.zpt**2 + df.zpn**2

    df['zmr'] = 1.e3*db.vp_r - 1e-9*db.br/np.sqrt(mu_0*1.e6*df.np * m_p)
    df['zmt'] = 1.e3*db.vp_t - 1e-9*db.bt/np.sqrt(mu_0*1.e6*df.np * m_p)
    df['zmn'] = 1.e3*db.vp_n - 1e-9*db.bn/np.sqrt(mu_0*1.e6*df.np * m_p)
    ...
    dfr['vA'] = 1e-9*dfr.bm/np.sqrt(mu_0*1.e6*dfr.np * m_p)

    # Assign the name of the file in which data is to be saved

    fn = save_dir + f[0][59:-8] + '_v01.hf'
...
fn = save_dir + fnames[0][-47:-8] + '_' + fnames[-1][-16:-8] + '_v03.p'

df_t.to_pickle(fn, protocol=2)

hdf=hf.File(fn,'w')
hdf.create_dataset('datetime',data=(pd.Series(df_t.index)-pd.datetime(1970,1,1,0,0,0,0, pytz.UTC)).dt.total_seconds())
