...
def plot_msc(key, scaled=True):
    ...
    if key == "b":
        ...
        dkey = "zpr"
        lab_name = r"$z^{+}_{r} = (v + b)_{r}$"
        lab_sym = r"Z^{+}_{r}"
        lab_unit = r"[]"
        corner = 3
        yscale = "log"
        ylim_scaled = [1e3, 1e5]
        ylim_unscaled = [1e3, 1e6]
        unit = 1.0
    ...
    fig_key.set_xlim([0.0, 4.05])  # Changed from 4. for New Horizons
    ...
    if key in ["loss", "betap"]:
    ...
    elif key in ["np", "vp", "vpr", "zpr", "zpt", "zpn"]:
        ...
        if key in ["va"]:
            ...
            bounds = (
                (0, 0.2, 1.0, -np.inf, -np.inf, -np.inf),
                (np.inf, 1.0, 10.0, np.inf, np.inf, np.inf),
            )
        elif key in ["tp"]:
            p0 = [1.0, 0.2, 30.0, 0.2, -0.7, 0.2]
        else:
...
#             dat = plot_msc(arr_key[i], scaled=scaled)
