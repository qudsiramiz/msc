...
def plot_indiv(key=None, fl=None, ylim=None, yscale=None, ylabel=None, keycorner=1):
    ...
    for i in range(len(lst_d)):
        ...
        fig_plot.plot((d, d), ylim, color=c, linewidth=0.5, linestyle=":")
    ...
    p1 = "/home/vetinari/Desktop/git/msc/git_data/binned/unscaled/v19/"
    p2 = "_coho1hr_merged_mag_plasma_"
    p3 = "_v07_80_binned_v19.hf"

    sc_fl = [
        p1 + "psp" + p2 + "20180101_20211001" + p3,
        ...
        p1 + "new_horizons" + p2 + "20081010_20200127" + p3,
    ...
    for sc in range(n_sc):
        ...
        dat_k = np.array(dat[key + "_iqr_50"])
        # dat_n = array(dat[key+'_num'])

        tk = np.where((np.isfinite(dat_r)) & (np.isfinite(dat_k)))[0]
        # (isfinite(dat_n)))[0]
    ...
    folder_name = "/home/vetinari/Desktop/git/msc/figures/indiv/"
    # Chrck if the folder exists, if not create it.
    pathlib.Path(folder_name).mkdir(parents=True, exist_ok=True)

    fname = "indiv_" + fl + "_v16.png"

    plt.savefig(f"{folder_name}{fname}", dpi=300, bbox_inches="tight", pad_inches=0.1)
