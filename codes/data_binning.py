...
def trace_func(df=None, key_list=None):
    ...
    df_omni_o = pd.read_pickle(
        "/mnt/cephadrius/udel_research/msc/omni/data/processed/v03/omni_coho1hr_merged_mag_plasma_19630101_20211201_v03.p"
    )
    ...
    t_omni = (
        pd.Series(df_omni.index) - pd.datetime(1970, 1, 1, 0, 0, 0, 0, pytz.UTC)
    ).dt.total_seconds()
...
for f in fnames[10:11]:
    ...
    if scaled:
        ...
        if binning:
            ...
            df_temp = bin_data(
                df_intrp,
                filename=save_dir1 + f[54:-2] + "_%s_binned_scaled" % (n_bin),
                filetype=["hdf", "pickle"],
                n_bin=n_bin,
...
if scaled:
    ...
    pd.DataFrame.to_pickle(
        df_all, save_dir + "all_spcaecraft_data_scaled_%s.p" % (version), protocol=2
    )
    dfn = bin_data(
        df_all,
        filename=save_dir + "all_spacecraft_data_binned_scaled",
        filetype=["hdf", "pickle"],
        n_bin=n_bin,
        file_version=file_version,
        key_list=key_list,
    ...
    dfn = bin_data(
        df_all,
        filename=save_dir + "all_spacecraft_data_binned",
        filetype=["hdf", "pickle"],
        n_bin=n_bin,
        file_version=file_version,
        key_list=key_list,
