...
data_dir =  '../data/merged_1hr/v06/all_lat_all_spc/'
...
if rebin_data:
    ...
    lat_bins = np.linspace(0, df.heliographicLatitude.max(), 80)

    Tp = np.full((len(r_bins), len(lat_bins)), np.nan)
    Tp_num = np.full((len(r_bins), len(lat_bins)), np.nan)
    for i in range(len(r_bins)-1):
        for j in range(len(lat_bins)-1):
            tk = np.where((df.sc_r >= r_bins[i]) & (df.sc_r < r_bins[i+1]) & (
                           abs(df.heliographicLatitude) >= lat_bins[j]) & (
                           abs(df.heliographicLatitude) < lat_bins[j+1]))[0]
            if len(tk) > 0:
...
for norm_plot in norm_plot_list:
    ...
    axs.pcolormesh(r_bins, lat_bins, np.transpose(plot_data), cmap='viridis',
                   norm=cbar_norm, alpha=0.5, shading="nearest", rasterized=True)

    axs.set_xlabel(r'$r$ (AU)', fontsize=labelsize)
    axs.set_ylabel(r'$\phi$ (deg)', fontsize=labelsize)
    axs.set_title(r'$T_p$')
    axs.set_xlim(r_bins[0], r_bins[-1])
    axs.set_ylim(lat_bins[0], lat_bins[-1])
    axs.set_xscale('log')
    ...
    plt.savefig(f"../figures/Tp_map_median_all_spc_density{int(norm_plot)}.pdf",
